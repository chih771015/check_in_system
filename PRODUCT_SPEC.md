...
- **離開後鎖定**：翻譯員一旦對該排班打了**離開卡**，就不能再上傳/刪除/標記診斷證明（回 `DIAGNOSIS_LOCKED_AFTER_LEAVE`，409）；前端離開後改為**唯讀**（completed 只剩「查看照片」），不顯示任何修改入口。之後僅**管理員**可代為更正——管理員在排班詳情對 completed 病人也有「管理照片」可增刪，不受離開鎖定限制。

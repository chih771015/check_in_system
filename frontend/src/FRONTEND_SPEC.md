...
| 統計事件匯流排 | `stores/statsEvents.ts` | 本檔 §5 |
...
| 工具 | `utils/apiError.ts`、`utils/schedulePatient.ts`、`utils/money.ts` | 本檔 §5 |
...
- `utils/money.ts`：`formatNT(amount?)` → `"NT$ 1,234"`（null/undefined 當 0）；金額顯示共用，避免各處手寫格式漂移。
- `stores/statsEvents.ts`：極小 pub/sub（`notifyActualAmountChanged`/`onActualAmountChanged`）。管理員改實付金額（`adminSetActualAmount`）後通知 `AppLayout` 橫幅刷新，不需把 callback 穿過整棵樹。
...
## 7. 測試考量
vitest + @testing-library/react；既有測試涵蓋 Login、AdminManagement、ChangePassword、PatientHistory、ScheduleManagement、DiagnosisResults、TranslatorManagement、各 component（含 AppLayout）、authStore、apiError、schedulePatient、money、i18n。E2E 見 `e2e/`。
- ⚠️ 前端型別檢查用 `npm run typecheck`（`tsc -b`，走 project references 檢查 `src` 含測試）；`npx tsc --noEmit` 因根 tsconfig `files:[]` 不檢查任何檔案。

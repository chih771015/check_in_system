...
1. 在 `backend/.env.production`（Docker）或 `backend/.env`（裸機）把 `DB_HOST/PORT/USER/PASSWORD/DB_NAME`
   改成你那顆 DB 的連線資訊（`DB_HOST` 填成 `postgres` 以外的值）。
2. **不用手動改 compose**。`deploy-prod.sh` 會**自動偵測** `DB_HOST`：
   只要不是 `postgres`，就不會啟動內建的 postgres 容器（內建 DB 定義在 overlay
   `docker-compose.localdb.yml`，偵測到外接 DB 時腳本就不疊加它）。
3. 確認後端所在網路連得到那顆 DB（防火牆 / VPC 安全群組放行 5432）。

> 小提醒：外接 DB 模式下 `DB_PASSWORD` 是「你那顆 DB 的真實密碼」，腳本**不會**幫你亂產，
> 留空會被擋下來。內建 DB 模式才會自動產生強密碼。
...
> **最保險**：直接抄 `deploy-prod.sh` 跑完結尾印出的「常用指令」——它已經幫你帶好正確的
> compose 檔（內建 DB 會自動含 `-f docker/docker-compose.localdb.yml`）。下面表格是通用版。

**內建 DB**（用了內建 postgres 容器）指令要**兩個檔一起帶**，否則 `down` 會漏掉 postgres：

```bash
DC="docker compose -f docker/docker-compose.prod.yml -f docker/docker-compose.localdb.yml"
```

**外接 DB** 則只要 base 一個檔：

```bash
DC="docker compose -f docker/docker-compose.prod.yml"
```

| 你想做的事 | Docker 指令 |
|-----------|------|
| 看服務狀態 | `$DC ps` |
| 看後端 log | `$DC logs -f backend` |
| 停止（保留資料） | `$DC down` |
| 升級（拉新 code 後） | `bash scripts/deploy-prod.sh` |
| 重設 admin 密碼 | `$DC exec backend ./server -reset-password admin@admin.com '新密碼'` |

> 若手動 `down` 時漏帶 `localdb` 檔導致 postgres 變孤兒，補一個
> `docker compose -f docker/docker-compose.prod.yml down --remove-orphans` 即可清掉。
